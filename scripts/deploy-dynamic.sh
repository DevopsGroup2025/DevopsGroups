...
# Full-stack playbook
readonly PLAYBOOK="${SCRIPT_DIR}/../ansible/deploy-fullstack.yml"

# Terraform directory (relative to project root)
readonly TERRAFORM_DIR="$(cd "${SCRIPT_DIR}/../terraform" && pwd)"

# Dynamic inventory file
readonly INVENTORY_FILE="${SCRIPT_DIR}/../ansible/aws_ec2.yml"
...
get_database_endpoint() {
    ...
    if ! db_host=$(terraform output -raw db_endpoint 2>/dev/null); then
        popd > /dev/null || true
...
test_dynamic_inventory() {
    ...
    log_info "Using inventory file: $INVENTORY_FILE"

    local inventory_json backend_count frontend_count

    echo "==== INVENTORY DEBUG OUTPUT ===="
    ansible-inventory -i "$INVENTORY_FILE" --list
    echo "==== END INVENTORY DEBUG OUTPUT ===="

    if ! inventory_json=$(ansible-inventory -i "$INVENTORY_FILE" --list 2>/dev/null); then
    ...
    echo "$inventory_json" > /tmp/ansible_inventory_debug.json
    log_info "Inventory JSON written to /tmp/ansible_inventory_debug.json"
...
display_deployment_info() {
    ...
    print_info "Inventory: Dynamic (aws_ec2.yml)"
    print_info "Playbook: $PLAYBOOK"
    print_info "AWS Region: $aws_region"
    print_info "Database Endpoint: $db_host"
...
run_deployment() {
    ...
    # Run ansible-playbook with inventory only (roles-path not supported)
    
    if ansible-playbook "$PLAYBOOK" -i "$INVENTORY_FILE" --private-key "$SCRIPT_DIR/../terraform/modules/compute/devops-team-key-pair.pem" -e "db_endpoint=$db_host"; then
        return 0
    else
        return 1
    fi
...
display_success_info() {
    ...
    if [[ -n "$lb_ip" && "$lb_ip" != "null" ]]; then
        print_info "  Load Balancer: ${GREEN}http://$lb_ip${NC}"
    else
        print_warn "  Load Balancer IP not available yet (check AWS console)"
    fi
    echo
